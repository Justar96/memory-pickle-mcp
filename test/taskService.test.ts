...
describe('TaskService', () => {
  ...
  describe('Line Range Functionality', () => {
    it('should create task with valid line_range', () => {
      const task = taskService.createTask({
      ...
      expect(task.title).toBe('Fix bug in authentication');
      expect(task.line_range).toBeDefined();
      expect(task.line_range?.start_line).toBe(45);
      expect(task.line_range?.end_line).toBe(67);
      expect(task.line_range?.file_path).toBe('src/auth/login.ts');
    });

    it('should create task without line_range (optional)', () => {
      const task = taskService.createTask({
        title: 'General task',
        description: 'A task without specific line references',
        project_id: 'proj_test'
      });

      expect(task.title).toBe('General task');
      expect(task.line_range).toBeUndefined();
    });

    it('should validate line_range parameters', () => {
      // Test invalid line numbers (negative)
      expect(() => taskService.createTask({
        title: 'Invalid task',
        project_id: 'proj_test',
        line_range: {
